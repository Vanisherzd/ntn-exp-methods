...
> **Correction, artifact-closure cycle (`76f53d3`).** Two of those six are *derived*, not frozen:
> `l47_power_curve.json` and `l47_calibration.json` are regenerated by `make matrix`. The closure
> cycle added `n_seeds_per_point` to the power curve, so it is no longer byte-identical to the
> hardened-final tag although all seven measured `halt_prob` values are unchanged. The four frozen
> artifacts — `l47_real_application.json`, `l47_alongtrack.json`, `paired.json`, `mechanical.json`
> — remain byte-identical. See "Artifact classes" in `paper/submission/FINAL_SUBMISSION_MANIFEST.md`.
>
> The same cycle **reverted** the widened `NOT OBSERVABLE` definition recorded in §3 below.
> Widening it to cover "the design cannot attain the nominal level" relocated a limit of *this
> study* onto the third-party artifact. `NOT OBSERVABLE` now means only that the inspected
> artifact does not expose the inputs the rule needs, and the five-seed downstream endpoint is
> reported as **not estimable at the available paired-run resolution**, a study-level evidence
> disposition rather than a rule verdict. §3 below records what this cycle did, not the final
> wording.
