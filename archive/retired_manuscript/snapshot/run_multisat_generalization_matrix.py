...
LEARNED_MODELS = ("linear_age", "age_ridge", "deployable_ridge")
...
# --------------------------------------------------------------------------
# DEPLOYMENT CAUSALITY (invariant I1/I2)
# --------------------------------------------------------------------------
# A deployed feature may use only: the stale TLE held by the terminal, the
# current transmission UTC, fixed ground-station parameters, static config, and
# frozen model parameters. Feature 1 (t_gap_s) is the stale->REFERENCE epoch
# gap: the reference element has not been published at transmission time, so it
# is NOT computable by the endpoint. Empirically proven by
# loop_engineering/evidence/T1_reference_epoch_perturbation.py, which swaps only
# the reference element and observes that exactly index 1 changes.
NON_DEPLOYABLE_FEATURE_INDICES = (1,)
DEPLOYABLE_FEATURE_INDICES = tuple(
    i for i in range(len(FEATURE_NAMES)) if i not in NON_DEPLOYABLE_FEATURE_INDICES
)


def assert_deployable(indices, who: str) -> tuple[int, ...]:
    """Reject any candidate that consumes a non-deployable feature (test T4)."""
    bad = sorted(set(indices) & set(NON_DEPLOYABLE_FEATURE_INDICES))
    if bad:
        names = ", ".join(FEATURE_NAMES[i] for i in bad)
        raise ValueError(
            f"deployment-causality violation in {who}: consumes {names} "
            f"(indices {bad}), which are not computable at the transmission epoch"
        )
    return tuple(indices)


def feature_manifest() -> list[dict[str, Any]]:
    """Machine-readable per-feature deployability record, persisted with runs."""
    return [
        {"index": i, "name": FEATURE_NAMES[i],
         "deployable": i not in NON_DEPLOYABLE_FEATURE_INDICES}
        for i in range(len(FEATURE_NAMES))
    ]


# Deployable candidate feature sets. age_ridge replaces the old stale_age_ridge,
# which consumed t_gap_s; deployable_ridge replaces the old full ridge.
AGE_ONLY_FEATURES = assert_deployable((0,), "age_ridge")
DEPLOYABLE_RIDGE_FEATURES = assert_deployable(
    DEPLOYABLE_FEATURE_INDICES, "deployable_ridge"
)
...
def fit_correctors(
    ...
    models["linear_age"] = (lambda c: (lambda x: c[0] * x[:, 0] + c[1]))(coef)

    models["age_ridge"], alpha_age = _fit_ridge_on(
        AGE_ONLY_FEATURES, x_tr, y_tr, x_va, y_va
    )
    models["deployable_ridge"], alpha_full = _fit_ridge_on(
        DEPLOYABLE_RIDGE_FEATURES, x_tr, y_tr, x_va, y_va
    )
    models["_alphas"] = {"age_ridge": alpha_age, "deployable_ridge": alpha_full}
    models["_manifest"] = {
        "linear_age": {"features": [0], "names": [FEATURE_NAMES[0]]},
        "age_ridge": {"features": list(AGE_ONLY_FEATURES),
                      "names": [FEATURE_NAMES[i] for i in AGE_ONLY_FEATURES]},
        "deployable_ridge": {"features": list(DEPLOYABLE_RIDGE_FEATURES),
                             "names": [FEATURE_NAMES[i]
                                       for i in DEPLOYABLE_RIDGE_FEATURES]},
    }
    return models
