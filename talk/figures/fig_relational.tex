...
\draw[rel] (a1) -- (a2);
\node[note] at ( 21.5,11) {same manifest, different output\\$\Rightarrow$ the manifest is incomplete};

\draw[black!15,line width=0.5pt] (46,32) -- (46,4);

\node[hdr] at ( 70.5,30) {across source states};
\node[box] (b1) at ( 57.5,20) {source\\at decision};
\node[box] (b2) at ( 83.5,20) {source\\later};
\draw[rel] (b1) -- (b2);
\node[note] at ( 70.5,11) {present in the later source,\\past-dated, yet unavailable\\at the decision time};
...
\draw[rel] (c1) -- (c2);
\node[note] at (119.5,11) {dependence at the coarser level\\$\Rightarrow$ the \emph{declared} unit is too fine};
\end{tikzpicture}
