...
# Fields whose RENDERED value legitimately differs between two correct runs of the same tree.
# Nothing else in the generated block may differ: check_banlist.py regenerates the table from the
# artifact and fails the build on any line that does not match, except where a field declared here
# sits. Declaring a field volatile is therefore an explicit, reviewable act -- the value is the
# documented reason.
#
# The regression GUARDS (runtime_threshold_s, runtime_threshold_ms_per_condition) are deliberately
# NOT here. They are promises the repository makes, not measurements, so moving 3 s to 2 s or 60 ms
# to 30 ms is semantic drift and must fail -- even though both appear in the SAME table row as the
# volatile measurements. That is why the comparison is field-level and not row-level: a
# runtime-row-wide exemption would have re-created exactly the hole it was meant to close.
VOLATILE_CLAIM_FIELDS: dict[str, str] = {
    "runtime_seconds": "wall clock; varies a few percent between runs and machines",
    "runtime_ms_per_condition": "derived from runtime_seconds; equally environment-dependent",
}

# Sentinel substituted for a volatile field to recover the row's invariant text. A NUL cannot occur
# in the artifact, so it can never collide with real content.
_VOLATILE_SENTINEL = "\x00VOLATILE\x00"
_NUMBER = r"-?[0-9]+(?:\.[0-9]+)?"


def claims_rows(s: dict) -> list[tuple[str, object, str]]:
    """The generated claims table as (claim, value, artifact field) triples.

    Pure: no file access, no mutation of `s`. The gate calls this to regenerate the table from
    the artifact rather than re-implementing the rendering, so the two cannot drift apart.
    """
    c = s["l47_calibration"]
...
def render_claims_block(s: dict) -> str:
    """The marker-delimited block exactly as it appears in CLAIMS.md."""
    body = ["| claim | value | artifact field |", "|---|---|---|"]
    body += [f"| {k} | {v} | `{f}` |" for k, v, f in claims_rows(s)]
    return (CLAIMS_BEGIN + "\n<!-- Generated by make matrix. Do not hand-edit: this file "
            "carried stale counts once. -->\n" + "\n".join(body) + "\n" + CLAIMS_END)


def claims_block_pattern(s: dict) -> str:
    """Regex that the checked-in generated block must FULLY match.

    Built by rendering the whole block a second time with every volatile field replaced by a
    sentinel: the surviving text is escaped literally and only the sentinel positions become free
    numbers. The comparison is therefore character-for-character everywhere except the exact
    offsets a declared volatile field occupies -- covering the claim names, the artifact-field
    column, the row order, the row count and the generator's own header comment, not just the
    value cells.

    Scoping this to the value cell alone was not enough, and a negative test proved it: swapping
    `clean_path_count` for `environment_count` in the artifact-field column left the value `3`
    intact, so a value-only check exempted the line and the substituted field passed. Which field
    backs a number is part of the claim.
    """
    masked = dict(s) | {k: _VOLATILE_SENTINEL for k in VOLATILE_CLAIM_FIELDS}
    parts = render_claims_block(masked).split(_VOLATILE_SENTINEL)
    return _NUMBER.join(re.escape(p) for p in parts)


def _write_claims_table(s: dict, path: Path | None = None) -> None:
    ...
    `path` exists so the gate can regenerate into a throwaway copy and diff it against the
    checked-in file without touching the repository.
    """
    path = CLAIMS if path is None else path
    if not path.exists():
        return
    t = path.read_text()
    if CLAIMS_BEGIN in t and CLAIMS_END in t:
        i, j = t.index(CLAIMS_BEGIN), t.index(CLAIMS_END) + len(CLAIMS_END)
        path.write_text(t[:i] + render_claims_block(s) + t[j:])
