...
def main() -> int:
    ...
    # n_seeds is read off the function's own signature rather than restated, so the count the
    # manuscript quotes for each power point cannot drift from the count that produced it.
    n_seeds = inspect.signature(power_curve).parameters["n_seeds"].default
    pc = power_curve(n_seeds=n_seeds)
