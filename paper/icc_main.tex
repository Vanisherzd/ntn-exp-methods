...
component whose error rates are measured rather than assumed. Around it, curated regression
coverage of seventeen fault classes in three deterministic environments
(Table~\ref{tab:rules}), which establishes that the represented violations cannot silently
return, not sensitivity to faults the suite lacks (\S\ref{sec:threats}).
\end{enumerate}
...
Each object is the obligation one domain property of \S\ref{sec:whysat} imposes; the parenthetical names which. \emph{Feature availability} and the \emph{availability
clock} (i): a feature must be computable from state available at $t_d$, and selection must use
publication, not the descriptive clock. The adversarial case is not a future-dated feature---ordering catches that---but one built from two past timestamps, one published after $t_d$, silent because both lie in the past. \emph{Row membership} (iv): later
...
addresses. \emph{State channels} (vi): information also enters through the scaler, coefficients, a tracker's latent state, selected-model metadata~\cite{cawley2010selection} or the gate bit---six probed channels, an operational enumeration, not a completeness claim.
...
Both pipelines come from a \emph{stopped} programme, were built with chronological splitting and passed it. They motivate the model rather than evidence it: retrospective accounts predating the contract, so we report which rule \emph{names} each defect rather than a halt we ran; no artifact survives for the second; and the fixtures of \S\ref{sec:eval} were modelled on them, so Table~\ref{tab:rules} and this section are not independent. No performance quantity is reported.
...
interval of unheld state. Epoch~$\le$~publication is unchecked and sometimes false: across the \num{63727} catalogue
records used here the epoch leads publication in \qty{27.6}{\percent} of cases by a median of
\qty{5.1}{\hour}---but \emph{entirely} in one object of eleven, so this may be one ingest
pipeline's artifact rather than a catalogue property. The two-clock premise rests instead on the
median publication lag, \qty{1.68}{\hour}, which is positive for all eleven.
