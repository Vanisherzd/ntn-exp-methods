...
before the decision instant $t_d$ populates a registry whose rows are frozen and hashed, a
model and admission decision are fixed at the freeze, deployment reads only frozen state,
and retrospective labels close afterwards. The three \emph{dashed} paths are the point of
this paper: each is \emph{chronologically consistent} in the sense a train/validation/test
split checks---the timestamps are correctly ordered---so ordering cannot express them. The
\emph{dotted} path (\rulename{L1.3}) is the contrast case: it \emph{is} an ordering
violation, and the one class a chronological split already catches.}
\label{fig:contract}
\end{figure*}
