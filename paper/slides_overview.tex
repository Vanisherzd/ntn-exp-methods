...
\begin{frame}[plain]
  \vspace{0.12cm}
  \begin{center}
    {\Large\bfseries Evidence-Gated Timing/Frequency Control for}\\[2pt]
    {\Large\bfseries LR-FHSS Direct-to-Satellite IoT}
  \end{center}
  \vspace{0.18cm}
  \begin{center}
...
\begin{frame}{Real BLACK KITE Negative Result}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{center}
      \includegraphics[width=\linewidth,height=0.66\textheight,keepaspectratio]{figures/fig_bk_residual_talk.pdf}
    \end{center}

    \column{0.18\textwidth}
    \vspace{0.76cm}
    \infocard{ScopeGray}{GrayFill}{0.90\linewidth}{\scriptsize
      \textbf{Result}\\
...
\begin{frame}{Synthetic Stress: Software-Only Sanity Check}
  \begin{columns}[T,totalwidth=\textwidth]
    \column{0.73\textwidth}
    \begin{center}
      \includegraphics[width=\linewidth,height=0.64\textheight,keepaspectratio]{figures/fig_gate_behavior_talk.pdf}
    \end{center}
    ...
    \begin{tabular}{p{0.68\linewidth}p{0.20\linewidth}}
      ...
      \bottomrule
    \end{tabular}}\\[0.12cm]
    {\tiny\color{ScopeGray}\parbox{0.94\linewidth}{Software-only synthetic sanity check; not a real BLACK KITE improvement and not hardware or live-link validation.}}
  \end{columns}
...
\begin{frame}{Timing / Frequency Ablation (proxy, not packet result)}
  ...
  \end{center}
  \scopefooter{Software-only coverage proxies. \textbf{Learned bar = controlled gate-open synthetic regime}; on real BLACK KITE the gate closes.}
  \takeaway{Joint timing+frequency control improves the synthetic gate-open proxy only.}
\end{frame}

\begin{frame}{Conducted-IQ Sanity Check (Table IV Boundary)}
  \begin{columns}[T,totalwidth=\textwidth]
  ...
  \end{columns}
\scopefooter{\textbf{Claim boundary:} conducted IQ-level only; no packet/PER/PDR/CRC/gateway ACK/OTA/live-satellite or link-layer validation.}
\end{frame}
...
\begin{frame}{Main Limitation: Cross-Satellite Generalization}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \infocard{UnsafeRed}{RedFill}{0.94\linewidth}{\small
      \textbf{Current real-data result}\\[2pt]
      BLACK KITE-1/2 only. This is a real-data negative control, not proof of constellation-wide generalization.
    }\\[0.16cm]
    \infocard{GateGreen}{GreenFill}{0.94\linewidth}{\small
      \textbf{Why this supports the story}\\[2pt]
      Limited transfer is exactly why always-on ML is unsafe. The Evidence Gate should be tested next as a cross-satellite safety mechanism.
    }

    \column{0.42\textwidth}
    \infocard{PhysicsBlue}{BlueFill}{0.94\linewidth}{\small
      \textbf{Next software campaign}\\[4pt]
      1. Multi-satellite TLE stress test\\
      2. Cross-satellite transfer heatmap\\
      3. Gate open/close statistics\\
      4. Failure taxonomy
    }
  \end{columns}
  \scopefooter{Frame method as local chronological deploy/no-deploy validation, not train-once-deploy-everywhere generalization.}
\end{frame}
...
\begin{frame}{Contributions}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \infocard{ScopeGray}{GrayFill}{0.90\linewidth}{\centering\scriptsize
      ...
      {\textbf{2. Real negative +}\\\textbf{synthetic open}}\\[-1pt]
    ...
    \infocard{ScopeGray}{GrayFill}{0.90\linewidth}{\centering\scriptsize
      ...
      {\textbf{3. Timing/frequency}\\\textbf{endpoint proxies}}\\[-1pt]
      Guard, hop-bin, energy policy.}
    }
    ...
    \infocard{ScopeGray}{GrayFill}{0.90\linewidth}{\centering\scriptsize
      ...
      {\textbf{4. Bounded}\\\textbf{conducted-IQ evidence}}\\[-1pt]
      Measurement path only.}
    }
  \end{columns}
  \vspace{0.52cm}
  \begin{center}
